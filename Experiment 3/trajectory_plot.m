clc; clear; close all
%% load data
Data(1) = load('DataExtraction_8_25.mat');
Data(2) = load('DataExtraction_8_26.mat');
Data(3) = load('DataExtraction_9_02.mat');
number_e = 3;
number_session = 4;
...
for num_sessionn = 1 : 2
    ...
    for num_trial = 1 : Data(num_e).record(num_session).number_trial
        ...
        if move_type(1:3) == [3,1,2] 
            mo = 1;
        end
        if move_type(1:3) == [3,2,1] 
            mo = 2;
        end
        if move_type(1:3) == [3,2,3] 
            mo = 3;
        end
        if move_type(1:3) == [3,3,2] 
            mo = 4;
        end
...
for i = 1 : 4
    ...
    for lengthth = 1 : time_length
        ...
        for num_trial = 1 : all_good_trial(i)
            ...
            if abs(record_y(i,num_trial,lengthth) - yy(lengthth)) <= 2 * ay(lengthth)
    ...
    ylim([0,1])
    hold on
    x_begin = type_information(8+i,4); x_end = type_information(8+i,5);
    y_begin = type_information(8+i,6); y_end = type_information(8+i,7);
...
for num_sessionn = 1 : 2
    ...
    for num_trial = 1 : Data(num_e).record(num_session).number_trial
        ...
        x(1:time_length) = x(1:time_length);
        y(1:time_length) = y(1:time_length) + (y_begin - y_move);
        x_ave = (x_end - x_begin) / (time_length - 1); 
        y_ave = (y_end - y_begin) / (time_length - 1); 
        ...
        dis_angle_mse = atan(dis_mse/distances)*180/pi;
        if dis_angle_mse <= angle_control
            if move_type(1:3) == [4,1,2] 
                mo = 1;
            end
            if move_type(1:3) == [4,2,1] 
                mo = 2;
            end
            if move_type(1:3) == [4,2,3] 
                mo = 3;
            end
            if move_type(1:3) == [4,3,2] 
                mo = 4;
            end
            all_good_trial(mo) = all_good_trial(mo) + 1; 
            for lengthth = 1 : time_length
...
for i = 1 : 4
    ...
    for num_trial = 1 : all_good_trial(i)
            ...
            xx = atand(((mean(record_x(i,num_trial,1:lengthth))-0.5)  * 72) / 68)-1;
            yy = atand(((mean(record_y(i,num_trial,1:lengthth))-0.5)  * 40.5) / 90);
            scatter(xx,yy,10,cc,'filled','o','LineWidth',0.5);
            hold on
        
    end
    hold on;
    if i == 2 
        scatter(0,0,20,[0,0,0],'filled','o','LineWidth',2);
    end
