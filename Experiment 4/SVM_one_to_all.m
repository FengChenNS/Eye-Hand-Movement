function [y_predict,bate] = SVM_one_to_all(X_train, y_train, X_test)

    bate = 0;
    y_labels = unique(y_train);
    n_class = size(y_labels, 1);
    models = cell(n_class, 1);
    bate(1:length(X_train),1:n_class) = 0;
    ...
    bate(1:length(svm_train_x),1) = bate(1:length(svm_train_x),1) / n_class;
